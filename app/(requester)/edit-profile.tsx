...
const RequesterEditProfileScreen: React.FC = () => {
  ...
  // Type guard to check if user is a requester
  const isRequester = (user: any): user is Requester => {
    return user?.userType === 'requester';
  };

  // Get initial emergency contact values safely
  const getInitialEmergencyContactName = (): string => {
    if (isRequester(user) && user.emergencyContact?.name) {
      return user.emergencyContact.name;
    }
    return '';
  };

  const getInitialEmergencyContactPhone = (): string => {
    if (isRequester(user) && user.emergencyContact?.phoneNumber) {
      return user.emergencyContact.phoneNumber;
    }
    return '';
  };

  const getInitialEmergencyContactRelationship = (): string => {
    if (isRequester(user) && user.emergencyContact?.relationship) {
  ...
  const [emergencyContactRelationship, setEmergencyContactRelationship] = useState(
    getInitialEmergencyContactRelationship()
  );
  ...
  const validateField = (field: string, value: string): string | undefined => {
    switch (field) {
      ...
      case 'emergencyContactName':
        ...
        if (value && !/^[a-zA-Z\s]+$/.test(value)) return 'Name can only contain letters';
        break;
      ...
      case 'emergencyContactRelationship':
        if (value && value.length < 2) return 'Relationship must be at least 2 characters';
        break;
    }
    return undefined;
  };
  ...
  const validateForm = (): boolean => {
    ...
    if (hasEmergencyContact) {
      if (!emergencyContactName) {
        ...
        const nameError = validateField('emergencyContactName', emergencyContactName);
      ...
      if (!emergencyContactPhone) {
        ...
        const phoneError = validateField('emergencyContactPhone', emergencyContactPhone);
      ...
      if (!emergencyContactRelationship) {
        ...
        const relationshipError = validateField(
          'emergencyContactRelationship',
          emergencyContactRelationship
        );
        if (relationshipError) newErrors.emergencyContactRelationship = relationshipError;
      }
    }
          ...
          <View style={styles.formCard}>
            ...
            <View style={styles.section}>
              ...
              <View style={styles.infoBox}>
                ...
                <Text style={styles.infoText}>
                  Your blood type helps us match you with compatible donors when needed.
                </Text>
              </View>
            </View>
            ...
            <View style={styles.section}>
              ...
              <View style={styles.infoBox}>
                ...
                <Text style={styles.infoText}>
                  Emergency contact will be notified in case of urgent medical situations.
                </Text>
              </View>
            </View>
...
export default RequesterEditProfileScreen;