...
const ACTIONS_CARD_W = 120 + 12; // Action card width + gap
...
export default function HomeScreen() {
    ...
    const handlePrevAction = () => {
        userTouchRef.current = Date.now();
        const pages = Math.ceil(actions.length / 3);
        if (pages <= 1) return;
        let next = (actionsIdx.current - 1) % pages;
        if (next < 0) next = pages - 1;
        actionsIdx.current = next;
        actionsScrollRef.current?.scrollTo({ x: next * (ACTIONS_CARD_W * 3), animated: true });
    };
    ...
    const handleNextAction = () => {
        ...
        const pages = Math.ceil(actions.length / 3);
        if (pages <= 1) return;
        const next = (actionsIdx.current + 1) % pages;
        actionsIdx.current = next;
        actionsScrollRef.current?.scrollTo({ x: next * (ACTIONS_CARD_W * 3), animated: true });
    };
    ...
    useEffect(() => {
        ...
        const ACTION_PAGE_W = ACTIONS_CARD_W * 3; // Scroll 3 at a time
        ...
        const timer = setInterval(() => {
            // Quick Actions (3 items per page)
            if (actions.length > 1 && actionsScrollRef.current) {
                if (Date.now() - userTouchRef.current >= PAUSE_MS) {
                    const pages = Math.ceil(actions.length / 3);
                    if (pages > 1) {
                        const next = (actionsIdx.current + 1) % pages;
                        actionsIdx.current = next;
                        actionsScrollRef.current.scrollTo({ x: next * ACTION_PAGE_W, animated: true });
                    }
                }
            }
    ...
    }, [isDonor, recentRequests.length, availableDonors.length, bloodBankStocks.length, actions.length]);
                ...
                >
                    ...
                    <View style={styles.section}>
                        <View style={styles.sectionHdr}>
                            ...
                            {actions.length > 3 && (
                                <View style={{ flexDirection: 'row', gap: 8 }}>
                                    <TouchableOpacity onPress={handlePrevAction} style={styles.navArrowBtn}>
                                        <Ionicons name="chevron-back" size={16} color={brand.orange} />
                                    </TouchableOpacity>
                                    <TouchableOpacity onPress={handleNextAction} style={styles.navArrowBtn}>
                                        <Ionicons name="chevron-forward" size={16} color={brand.orange} />
                                    </TouchableOpacity>
                                </View>
                            )}
                        </View>
                        <ScrollView
                            ...
                            snapToInterval={ACTIONS_CARD_W * 3}
                            decelerationRate="fast"
                            disableIntervalMomentum={true}
                            onScrollBeginDrag={() => { userTouchRef.current = Date.now(); }}
                            onScrollEndDrag={() => { userTouchRef.current = Date.now(); }}
                            scrollEventThrottle={16}
                        >
                            {actions.map((a, i) => (
                                <TouchableOpacity key={`action-${i}`} style={[styles.actionCard, { width: 120 }]} onPress={() => router.push(a.route as any)} activeOpacity={0.75}>
                        ...
                        {!isDonor && (
                            <View style={styles.actionsRow3}>
                                ...
                                >
                                    ...
                                    <View style={{ flex: 1 }}>
                                        <Text style={[styles.actionTitle, { textAlign: 'left' }]} numberOfLines={1}>Health Guide</Text>
                                        <Text style={[styles.actionDesc, { textAlign: 'left' }]} numberOfLines={1}>Tips & info</Text>
                                    </View>
                                </TouchableOpacity>
                    ...
                    <DashboardInsights
                        ...
                        onSeeAllDemand={() => router.push('/(shared)/blood-demand' as any)}
...
const getStyles = (colors: any, insets: any, isDark: boolean, brand: any) => StyleSheet.create({
    ...
    navArrowBtn: { width: 28, height: 28, borderRadius: 14, backgroundColor: 'rgba(234, 88, 12, 0.12)', justifyContent: 'center', alignItems: 'center' },
