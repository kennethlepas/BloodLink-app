...
const MyRequestsScreen: React.FC = () => {
  ...
  const getUrgencyConfig = (u: string) => {
    switch (u) {
      ...
      default: return { color: colors.textMuted, bg: isDark ? 'rgba(59, 130, 246, 0.15)' : '#DBEAFE', icon: 'information-circle', label: 'MODERATE' };
    }
  };
  ...
  const st = StyleSheet.create({
    ...
    // Card
    card: { backgroundColor: colors.surface, borderRadius: 14, marginBottom: 12, overflow: 'hidden', borderWidth: 1, borderColor: colors.divider, ...shadow('#000', 0.08, 10, 3) },
    ...
    modalSheet: { borderRadius: 26, padding: 24, paddingBottom: 30, ...shadow('#000', 0.2, 30, 10) },
  ...
  const renderRequestListItem = ({ item }: { item: BloodRequest }) => {
    ...
    return (
      <TouchableOpacity style={st.card} onPress={() => setViewRequest(item)} activeOpacity={0.9}>
        <LinearGradient
          colors={item.status === 'pending' ? [colors.success, '#34D399'] : item.status === 'completed' ? [colors.primary, '#059669'] : ['#334155', '#475569']}
        ...
        <div style={{ padding: 14, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          <View>
            <Text style={{ fontSize: 11, color: colors.textMuted, fontWeight: '700', textTransform: 'uppercase', marginBottom: 2 }}>Patient</Text>
            <Text style={{ fontSize: 15, color: colors.text, fontWeight: '700' }}>{item.patientName}</Text>
          </View>
          <View style={{ alignItems: 'flex-end' }}>
            <Text style={{ fontSize: 11, color: colors.textMuted, fontWeight: '700', textTransform: 'uppercase', marginBottom: 2 }}>Blood</Text>
    ...
    <SafeAreaView style={st.container} edges={['top']}>
      ...
      <Modal visible={!!viewRequest} transparent animationType="fade" onRequestClose={() => setViewRequest(null)}>
        <View style={[st.modalOverlay, { backgroundColor: colors.drawerOverlay }]}>
          <View style={[st.modalSheet, { maxHeight: '85%', backgroundColor: colors.surface }]}>
            <View style={[st.modalHandle, { backgroundColor: colors.surfaceBorder }]} />
            <View style={[st.modalHeaderRow, { marginBottom: 10, paddingBottom: 10, borderBottomWidth: 1, borderBottomColor: colors.divider }]}>
              <Text style={[st.modalTitle, { color: colors.text }]}>Request Details</Text>
              <TouchableOpacity onPress={() => setViewRequest(null)} style={[st.modalCloseBtn, { backgroundColor: colors.surfaceAlt }]}>
